...
"""Online-memory adaptation for the real RoboCasa Atomic-5 Stage-2 policy.

The Stage-2 behavior path and Cosmos backbone are loaded from the verified
Atomic-5 Stage-2 checkpoint and frozen.  The separately trained
history-only ``OnlineMemoryEncoder`` is consumed through the cached
``behavior_online_context`` token; this recipe trains the zero-init Cosmos
online projector so that the new context affects only the action branch.
"""
...
# The already trained PBD/adapter/global projector are deliberately not added
...
dataset["behavior_phase_cache"] = "${oc.env:BEHAVIOR_PHASE_CACHE}"
dataset["behavior_online_context_cache"] = "${oc.env:BEHAVIOR_ONLINE_CONTEXT_CACHE}"
